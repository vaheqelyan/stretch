...
impl Stretch {
    ...
    pub fn new_node(&mut self, style: Style, children: Vec<Node>) -> Result<NodeId, Error> {
        let node = self.allocate_node();
        let children = children.iter().map(|child| self.find_node(*child)).collect::<Result<Vec<_>, Error>>()?;
...
//         self.add_node(node, id);
    ...
    pub fn set_pos(&mut self, node: Node, x: f32, y: f32, bottom: f32, right: f32, offset_in: f32) -> Result<(), Error> {
        let id = self.find_node(node)?;
        self.forest.set_pos(id, x, y, bottom, right, offset_in);
        Ok(())
    }
...
//         Ok( node )
    ...
    /*pub fn is_text_node(&self, node: Node) -> Result<bool, Error> {
        let id = self.find_node(node)?;
        Ok(self.forest.nodes[id].is_text)
    }*/
    
    /*pub fn get_text_value(&self, node: Node) -> Result<String, Error> {
        let id = self.find_node(node)?;
        Ok(self.forest.nodes[id].text_value.to_string())
    }*/
    ...
    pub fn print_nodes(&mut self) {
        for x in self.nodes_to_ids.iter() {
            println!("{:?}", x);
        }
